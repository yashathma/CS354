...
//  -------------------------------------------------------------------
//  Date: 3/29/2024
//  Commit Message:  Finished whole program. Was having trouble with how to
//                  implement LRU but got it working a few hours later.
// 
// 
...
//Added Glpbal Variables
//Tracks the time for when the line was accessed.
...
void init_cache() {    
    ...
    if (!cache) {
        fprintf(stderr, "Memory allocation error for cache sets.\n");
        exit(1);
    }

    // For each set, allocate memory for E lines and initialize them
    for (set_index = 0; set_index < S; set_index++) {
        cache[set_index] = (cache_line_t*)malloc(sizeof(cache_line_t) * E);
        
        if (!cache[set_index]) {
            fprintf(stderr, "Memory allocation error for cache lines in set %d.\n", set_index);
            exit(1);
        }

        // Initialize all lines in this set
        for (int line_index = 0; line_index < E; line_index++) {
            cache[set_index][line_index].valid = 0;  // Set valid bit to 0
            cache[set_index][line_index].tag = 0;    // Set tag to 0
            cache[set_index][line_index].access_time = 0;
...
void access_data(mem_addr_t addr) {
    unsigned long long int tag = addr >> (s + b); // Calculate tag
    int set_index = (addr >> b) & ((1 << s) - 1); // Calculate set index

    bool hit = false;
    bool found_empty_line = false;
    int empty_line_index = 0; // To keep track of the first empty line encountered
    int lru_index = 0; // To keep track of the least recently used line
    unsigned long long int lru_counter = ULLONG_MAX; // Max value for comparison
    ...
    for (int i = 0; i < E; i++) {
        if (cache[set_index][i].valid) {
            if (cache[set_index][i].tag == tag) {
                ...
                hit = true;
                hit_cnt++;
                cache[set_index][i].access_time = ++global_counter; // Update LRU
                break;
            }

            // Update LRU info
            if (cache[set_index][i].access_time < lru_counter) {
                lru_counter = cache[set_index][i].access_time;
                lru_index = i;
            }
        } else if (!found_empty_line) {
            // Found an empty line, record it but keep checking for a hit
    ...
    if (!hit) {
        ...
        if (found_empty_line) {
            // Use the first empty line found if available
            lru_index = empty_line_index;
        } else {
            // No empty lines found; an eviction is needed
            evict_cnt++;
        }
        
        // Load data into cache (set tag, valid bit, and LRU info)
        cache[set_index][lru_index].tag = tag;
        cache[set_index][lru_index].valid = 1;
        cache[set_index][lru_index].access_time = ++global_counter;
    }
}
